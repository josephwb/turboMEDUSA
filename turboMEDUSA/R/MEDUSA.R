MEDUSA <- function(phy, richness=NULL, model="mixed", modelLimit=20, stop="threshold",
	shiftCut="both", criterion="aicc", preserveModelFlavour=FALSE, epsilon=NULL, r=NULL,
	b=NULL, d=NULL, constraint="fixed", initialR=0.05, initialE=0.5, plotFig=FALSE, verbose=TRUE,
	mc=FALSE, numCores=NULL, ...)
{
...
# Select model with best score according to the specific criterion employed (default aicc)
	...
	results <- list(z=z.orig, desc=desc, models=models, phy=phy, fixedPar=fixPar, threshold=threshold, modelSummary=modelSummary);
