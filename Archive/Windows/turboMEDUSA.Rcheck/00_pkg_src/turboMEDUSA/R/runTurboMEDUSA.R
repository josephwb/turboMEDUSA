...
## The important bits. Set up z, get descendants and number of tips per node
	obj <- make.cache.medusa(phy=phy, richness=richness, all.nodes=all.nodes);
...
# Select model with best score according to the specific criterion employed (default aicc)
	...
	modelSummary <- calculate.model.fit.summary(models=models, phy=phy, plotFig=ifelse(length(models) > 1 & plotFig, TRUE, FALSE), threshold=threshold);
	if (verbose)
	{
		cat("\n", "Model fit summary:", "\n\n", sep="");
		print(modelSummary);
